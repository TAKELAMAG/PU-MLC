...
class AveragePrecisionMeter(object):
    ...
    def add(self, output, target):
        ...
        if self.scores.untyped_storage().size() < self.scores.numel() + output.numel():
            newSize = math.ceil(self.scores.untyped_storage().size() * 1.5)
            self.scores.untyped_storage().resize_(int(newSize + output.numel()))
            self.targets.untyped_storage().resize_(int(newSize + output.numel()))
    ...
    def overall_topk(self, k, threshold):
    ...
    def evaluation(self, scores_, targets_, threshold):
        ...
        for index in range(classNum):
            ...
            Np[index] = np.sum(scores >= threshold)
            Nc[index] = np.sum(targets * (scores >= threshold))
